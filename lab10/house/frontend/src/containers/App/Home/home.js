...
import { clearMessage } from '../../../store/houseSlice';
  const Home = () => {
    ...
    const handleAddToCart = (id) => {
      dispatch(addToCartBackend(id))
        .unwrap()
        .catch((err) => {
          console.error('Error adding house to cart:', err); // Log the full error
          message.error('Failed to add house to cart');
        });
    };
          ...
          {houses.length > 0 ? (
            houses.map((house) => (
              <CardItem
                ...
                onAddToCart={() => handleAddToCart(house.id)}
              />
            ))
          ) : (
            <p>No houses found.</p>
